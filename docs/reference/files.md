...
| `monitor/skeptic-channel.sh` | The worker↔skeptic comms channel + `await` park used by `nexus.skeptic` (await-hang marker under `.state/skeptic/pending/`; per-round identity stamp at `.state/skeptic/<window>/.round`, read by `ng wrap-up`'s idempotency guard — `round` / `reopen`) | yes |
