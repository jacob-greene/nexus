...
1. **`monitor/watcher/main.sh`** — long-running watcher loop. Runs
   ...
   - **idle-worker transitions** in a `--- idle workers ---`
     ...
     | `over-limit` | `<window> OVER-LIMIT (resets <reset_at>; weekly Opus limit hit — schedule resume)` | Worker's claude session hit the weekly Opus limit and is functionally suspended. Do NOT close. The **watcher** owns the wake-loop (issue #87): stamps a row in `monitor/.state/over-limit-state.tsv`, retries on exponential backoff (60s → 300s cap, default 10 attempts), pastes a resume brief into the pane the moment `pane-state.sh` reports the suspension cleared. While the orchestrator's own pane is in this state, the watcher also suppresses routine emit-pastes to it (they'd queue uselessly in an inert input box) — archives still write. The suppression is bounded three ways, each independent of the others: the row's deadline is frozen at first sight and never recomputed from the dateless reset token; the pause predicate stops holding once that deadline is `monitor.over_limit.stale_grace_seconds` old (default 1800); and the absolute ceiling `monitor.over_limit.max_hold_seconds` (default 90000) is evaluated on every wake tick, before the due-gate. All three exist because a single latched row suppressed 889 emits over 6h36m on 2026-09-02. `reset_at` is a single token (spaces→`_`, parens stripped). Inviolable — never suppressed by `window-retain`. |
